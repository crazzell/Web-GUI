<%@ taglib uri="/struts-tags" prefix="s"%>    

  <ul>
    <li>Name: 
      <s:a href="http://luckit.biz/new/?tcp_product=waveio" target="blank_">
        WaveIO Asynchronous USB-to-I2S converter</s:a></li>
    <li>Driver: snd-usb-audio</li>
    <li>Max Rate: 192k</li>
    <li>Class: UAC2</li>
    <li>Used by: JackOfAll</li>
  </ul>
  <br />

  <ul>
    <li>Name: 
      <s:a href="http://amanero.com" target="blank_">
        Amanero Combo384 Module - USB class 2 to I2S 32bit and DSD output 
        Adapter</s:a></li>
    <li>Driver: snd-usb-audio</li>
    <li>Max Rate: 384k</li>
    <li>Class: UAC2</li>
    <li>Used by: JackOfAll</li>
  </ul>
  <br />

  <ul>
    <li>Name: 
      <s:a href="http://www.m2tech.biz/hiface.html" target="blank_">
        hiFace Hi-End S/PDIF Output Interface</s:a></li>
    <li>Driver: snd-usb-hiface</li>
    <li>Max Rate: 192k</li>
    <li>Class: UAC2</li>
    <li>Used by: agillis</li>
  </ul>
  <br />

  <ul>
    <li>Name: <s:a href="http://www.audiolab.co.uk/M-DAC%20Series.aspx?lang=En" target="blank_">
      Audiolab M-Dac</s:a> [Lakewest audio software]</li>
    <li>Driver: snd-usb-audio</li>
    <li>Max Rate: 96k</li>
    <li>Class: UAC1</li>
    <li>Hub: Bus powered external high speed hub</li>
    <li>Used by: Triode</li>
  </ul>
  <br />

  <ul>
    <li>Name: <s:a href="http://www.beresford.me" target="blank_">
      Beresford Caiman DAC Gatorized - TC-7520SEG</s:a></li>
    <li>Driver: snd-usb-audio</li>
    <li>Max Rate: 96k</li>
    <li>Class: UAC1</li>
    <li>Used by: lintweaker</li>
  </ul>
  <br />
  
  <ul>
    <li>Name: <s:a href="http://www.cambridgeaudio.com/products/dacmagic-plus-digital-to-analogue-converter" 
                   target="blank_">Cambridge Audio DacMagic Plus</s:a></li>
    <li>Driver: snd-usb-audio</li>
    <li>Max Rate: 96k/192k</li>
    <li>Class: UAC1/UAC2 (UAC mode is switchable)</li>
    <li>Used by: lintweaker</li>
  </ul>
  <br />

  <ul>
    <li>Name: <s:a href="http://mytekdigital.com/hifi" 
                   target="blank_">Mytek Digital Stereo192-DSD DAC</s:a></li>
    <li>Driver: <s:a href="https://github.com/lintweaker/mytekusb2" 
                     target="blank_">snd-usb-audio</s:a></li>
    <li>Max Rate: 96k/192k</li>
    <li>Class: UAC1/UAC2 (The Mytek Digital has both a USB1.1 and a USB2 connector)</li>
    <li>Used by: lintweaker</li>
  </ul>
  <br />
  
  <ul>
    <li>Name: <s:a href="http://www.sonore.us/index2.html" 
                   target="blank_">Sonore ExD DAC</s:a></li>
    <li>Driver: snd-usb-audio</li>
    <li>Max Rate: 384k</li>
    <li>Class: UAC2</li>
    <li>Used by: lintweaker</li> 
  </ul>
  <br />
  
  <ul>
    <li>Name: <s:a href="http://www.littledot.net/forum/viewtopic.php?f=9&t=814&sid=5f6cdbb7b861234e2f2753d134177f66" 
                   target="blank_">Little Dot DAC 1</s:a></li>
    <li>Max Rate: 96k</li>
    <li>Class: UAC1</li>
    <li>Used by: albertone74</li>
  </ul>
  <br />

  <ul>
    <li>Name: Cambridge Audio DacMagic</li>
    <li>Max Rate: 96k</li>
    <li>Class: UAC1</li>
    <li>Used by: albertone74</li>
  </ul>
  <br />
  
  <ul>
    <li>Name: <s:a href="http://www.musicalfidelity.com/products/vSeries/v-link192/" 
                   target="blank_">Musical Fidelity V-192 S/PDIF Output Interface</s:a></li>
    <li>Driver: snd-usb-audio</li>
    <li>Max Rate: 192k</li>
    <li>Class: UAC2</li>
    <li>Used by: dsdreamer</li>
  </ul>
  <br />

  <ul>
    <li>Name: <s:a href="http://hifimediy.com/index.php?route=product/product&product_id=83" 
                   target="blank_">HifiMeDiy Sabre USB DAC</s:a></li>
    <li>Driver: snd-usb-audio</li>
    <li>Max Rate: 96k</li>
    <li>Class: UAC2 (TenorTE7022L USB chip)</li>
    <li>Used by: Stoker</li> 
  </ul>
  <br />
